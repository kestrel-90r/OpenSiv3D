...
namespace s3d
{
	GLES3Renderer3DCommandManager::GLES3Renderer3DCommandManager()
	{
	...
	void GLES3Renderer3DCommandManager::reset()
	{
		// clear commands
		{
			m_commands.clear();
		...
		// Begin a new frame
		{
			...
			m_commands.emplace_back(GLES3Renderer3DCommandType::RasterizerState, 0);
			m_currentRasterizerState = m_rasterizerStates.back();

			m_commands.emplace_back(GLES3Renderer3DCommandType::DepthStencilState, 0);
			m_currentDepthStencilState = m_depthStencilStates.back();

			for (uint32 i = 0; i < SamplerState::MaxSamplerCount; ++i)
			{
				const auto command = ToEnum<GLES3Renderer3DCommandType>(FromEnum(GLES3Renderer3DCommandType::VSSamplerState0) + i);
			...
			for (uint32 i = 0; i < SamplerState::MaxSamplerCount; ++i)
			{
				const auto command = ToEnum<GLES3Renderer3DCommandType>(FromEnum(GLES3Renderer3DCommandType::PSSamplerState0) + i);
				m_commands.emplace_back(command, 0);
				m_currentPSSamplerStates[i] = m_currentPSSamplerStates.back();
			}

			m_commands.emplace_back(GLES3Renderer3DCommandType::ScissorRect, 0);
			m_currentScissorRect = m_scissorRects.back();

			m_commands.emplace_back(GLES3Renderer3DCommandType::Viewport, 0);
			...
			m_commands.emplace_back(GLES3Renderer3DCommandType::SetPS, 0);
			m_currentPS = PixelShader::IDType::InvalidValue();

			m_commands.emplace_back(GLES3Renderer3DCommandType::CameraTransform, 0);
			m_currentCameraTransform = m_cameraTransforms.back();

			m_commands.emplace_back(GLES3Renderer3DCommandType::EyePosition, 0);
...

			{
				for (uint32 i = 0; i < SamplerState::MaxSamplerCount; ++i)
				{
					const auto command = ToEnum<GLES3Renderer3DCommandType>(FromEnum(GLES3Renderer3DCommandType::VSTexture0) + i);
...

			{
				for (uint32 i = 0; i < SamplerState::MaxSamplerCount; ++i)
				{
					const auto command = ToEnum<GLES3Renderer3DCommandType>(FromEnum(GLES3Renderer3DCommandType::PSTexture0) + i);
	...
	void GLES3Renderer3DCommandManager::flush()
	{
		...
		if (m_changes.has(GLES3Renderer3DCommandType::BlendState))
		{
			m_commands.emplace_back(GLES3Renderer3DCommandType::BlendState, static_cast<uint32>(m_blendStates.size()));
			m_blendStates.push_back(m_currentBlendState);
		}

		if (m_changes.has(GLES3Renderer3DCommandType::RasterizerState))
		{
			m_commands.emplace_back(GLES3Renderer3DCommandType::RasterizerState, static_cast<uint32>(m_rasterizerStates.size()));
			m_rasterizerStates.push_back(m_currentRasterizerState);
		}

		if (m_changes.has(GLES3Renderer3DCommandType::DepthStencilState))
		{
			m_commands.emplace_back(GLES3Renderer3DCommandType::DepthStencilState, static_cast<uint32>(m_depthStencilStates.size()));
		...
		if (m_changes.has(GLES3Renderer3DCommandType::ScissorRect))
		{
			m_commands.emplace_back(GLES3Renderer3DCommandType::ScissorRect, static_cast<uint32>(m_scissorRects.size()));
			m_scissorRects.push_back(m_currentScissorRect);
		}

		if (m_changes.has(GLES3Renderer3DCommandType::Viewport))
		{
			m_commands.emplace_back(GLES3Renderer3DCommandType::Viewport, static_cast<uint32>(m_viewports.size()));
			m_viewports.push_back(m_currentViewport);
		}

		//if (m_changes.has(GLES3Renderer2DCommandType::SDFParams))
		//{
		...
		if (m_changes.has(GLES3Renderer3DCommandType::SetVS))
		{
			m_commands.emplace_back(GLES3Renderer3DCommandType::SetVS, static_cast<uint32>(m_VSs.size()));
			m_VSs.push_back(m_currentVS);
		}

		if (m_changes.has(GLES3Renderer3DCommandType::SetPS))
		{
			m_commands.emplace_back(GLES3Renderer3DCommandType::SetPS, static_cast<uint32>(m_PSs.size()));
			m_PSs.push_back(m_currentPS);
		}

		if (m_changes.has(GLES3Renderer3DCommandType::CameraTransform))
		{
			m_commands.emplace_back(GLES3Renderer3DCommandType::CameraTransform, static_cast<uint32>(m_cameraTransforms.size()));
			m_cameraTransforms.push_back(m_currentCameraTransform);
		}

		if (m_changes.has(GLES3Renderer3DCommandType::SetConstantBuffer))
		{
			assert(not m_constantBufferCommands.isEmpty());
			m_commands.emplace_back(GLES3Renderer3DCommandType::SetConstantBuffer, static_cast<uint32>(m_constantBufferCommands.size()) - 1);
		}

		if (m_changes.has(GLES3Renderer3DCommandType::EyePosition))
		{
			m_commands.emplace_back(GLES3Renderer3DCommandType::EyePosition, static_cast<uint32>(m_eyePositions.size()));
			m_eyePositions.push_back(m_currentEyePosition);
		}

		if (m_changes.has(GLES3Renderer3DCommandType::LocalTransform))
		{
		...
		if (m_changes.has(GLES3Renderer3DCommandType::SetMesh))
		{
			m_commands.emplace_back(GLES3Renderer3DCommandType::SetMesh, static_cast<uint32>(m_meshes.size()));
	...
	void GLES3Renderer3DCommandManager::pushDraw(const uint32 startIndex, const uint32 indexCount, const PhongMaterialInternal& material, const uint32 instanceCount)
	{
		// [Siv3D ToDo]
	...
	const GLES3Draw3DCommand& GLES3Renderer3DCommandManager::getDraw(const uint32 index) const noexcept
	{
	...
	void GLES3Renderer3DCommandManager::pushBlendState(const BlendState& state)
	{
		constexpr auto command = GLES3Renderer3DCommandType::BlendState;
	...
	const BlendState& GLES3Renderer3DCommandManager::getBlendState(const uint32 index) const
	{
		return m_blendStates[index];
	}

	const BlendState& GLES3Renderer3DCommandManager::getCurrentBlendState() const
	{
		return m_currentBlendState;
	}

	void GLES3Renderer3DCommandManager::pushRasterizerState(const RasterizerState& state)
	{
		constexpr auto command = GLES3Renderer3DCommandType::RasterizerState;
	...
	const RasterizerState& GLES3Renderer3DCommandManager::getRasterizerState(const uint32 index) const
	{
		return m_rasterizerStates[index];
	}

	const RasterizerState& GLES3Renderer3DCommandManager::getCurrentRasterizerState() const
	{
		return m_currentRasterizerState;
	}

	void GLES3Renderer3DCommandManager::pushDepthStencilState(const DepthStencilState& state)
	{
		constexpr auto command = GLES3Renderer3DCommandType::DepthStencilState;
	...
	const DepthStencilState& GLES3Renderer3DCommandManager::getDepthStencilState(const uint32 index) const
	{
		return m_depthStencilStates[index];
	}

	const DepthStencilState& GLES3Renderer3DCommandManager::getCurrentDepthStencilState() const
	{
		return m_currentDepthStencilState;
	}

	void GLES3Renderer3DCommandManager::pushVSSamplerState(const SamplerState& state, const uint32 slot)
	{
		assert(slot < SamplerState::MaxSamplerCount);

		const auto command = ToEnum<GLES3Renderer3DCommandType>(FromEnum(GLES3Renderer3DCommandType::VSSamplerState0) + slot);
	...
	const SamplerState& GLES3Renderer3DCommandManager::getVSSamplerState(const uint32 slot, const uint32 index) const
	{
		assert(slot < SamplerState::MaxSamplerCount);
	...
	const SamplerState& GLES3Renderer3DCommandManager::getVSCurrentSamplerState(const uint32 slot) const
	{
		assert(slot < SamplerState::MaxSamplerCount);
	...
	void GLES3Renderer3DCommandManager::pushPSSamplerState(const SamplerState& state, const uint32 slot)
	{
		assert(slot < SamplerState::MaxSamplerCount);

		const auto command = ToEnum<GLES3Renderer3DCommandType>(FromEnum(GLES3Renderer3DCommandType::PSSamplerState0) + slot);
	...
	const SamplerState& GLES3Renderer3DCommandManager::getPSSamplerState(const uint32 slot, const uint32 index) const
	{
		assert(slot < SamplerState::MaxSamplerCount);
	...
	const SamplerState& GLES3Renderer3DCommandManager::getPSCurrentSamplerState(const uint32 slot) const
	{
		assert(slot < SamplerState::MaxSamplerCount);
	...
	void GLES3Renderer3DCommandManager::pushScissorRect(const Rect& state)
	{
		constexpr auto command = GLES3Renderer3DCommandType::ScissorRect;
	...
	const Rect& GLES3Renderer3DCommandManager::getScissorRect(const uint32 index) const
	{
		return m_scissorRects[index];
	}

	const Rect& GLES3Renderer3DCommandManager::getCurrentScissorRect() const
	{
		return m_currentScissorRect;
	}

	void GLES3Renderer3DCommandManager::pushViewport(const Optional<Rect>& state)
	{
		constexpr auto command = GLES3Renderer3DCommandType::Viewport;
	...
	const Optional<Rect>& GLES3Renderer3DCommandManager::getViewport(const uint32 index) const
	{
		return m_viewports[index];
	}

	const Optional<Rect>& GLES3Renderer3DCommandManager::getCurrentViewport() const
	{
		return m_currentViewport;
	}

	//void GLES3Renderer2DCommandManager::pushSDFParameters(const std::array<Float4, 3>& state)
	//{
	//	constexpr auto command = GLES3Renderer2DCommandType::SDFParams;
	...
	//const std::array<Float4, 3>& GLES3Renderer2DCommandManager::getSDFParameters(const uint32 index) const
	//{
	//	return m_sdfParams[index];
	//}

	//const std::array<Float4, 3>& GLES3Renderer2DCommandManager::getCurrentSDFParameters() const
	//{
	...
	{
		constexpr auto command = GLES3Renderer3DCommandType::SetVS;
	...
	void GLES3Renderer3DCommandManager::pushCustomVS(const VertexShader& vs)
	{
		const auto id = vs.id();
		constexpr auto command = GLES3Renderer3DCommandType::SetVS;
		auto& current = m_currentVS;
	...
	void GLES3Renderer3DCommandManager::pushStandardPS(const PixelShader::IDType& id)
	{
		constexpr auto command = GLES3Renderer3DCommandType::SetPS;
	...
	void GLES3Renderer3DCommandManager::pushCustomPS(const PixelShader& ps)
	{
		const auto id = ps.id();
		constexpr auto command = GLES3Renderer3DCommandType::SetPS;
		auto& current = m_currentPS;
	...
	void GLES3Renderer3DCommandManager::pushCameraTransform(const Mat4x4& state)
	{
		constexpr auto command = GLES3Renderer3DCommandType::CameraTransform;
	...
	const Mat4x4& GLES3Renderer3DCommandManager::getCurrentCameraTransform() const
	{
		return m_currentCameraTransform;
	}

	const Mat4x4& GLES3Renderer3DCommandManager::getCameraTransform(const uint32 index) const
	{
		return m_cameraTransforms[index];
	}

	void GLES3Renderer3DCommandManager::pushEyePosition(const Float3& state)
	{
		constexpr auto command = GLES3Renderer3DCommandType::EyePosition;
	...
	const Float3& GLES3Renderer3DCommandManager::getCurrentEyePosition() const
	{
		return m_currentEyePosition;
	}

	const Float3& GLES3Renderer3DCommandManager::getEyePosition(const uint32 index) const
	{
		return m_eyePositions[index];
	}

	void GLES3Renderer3DCommandManager::pushLocalTransform(const Mat4x4& state)
	{
		constexpr auto command = GLES3Renderer3DCommandType::LocalTransform;
	...
	const Mat4x4& GLES3Renderer3DCommandManager::getCurrentLocalTransform() const
	{
	...
	{
		constexpr auto command = GLES3Renderer3DCommandType::SetConstantBuffer;
		...
		GLES3ConstantBuffer3DCommand cb
		{
			.stage = stage,
			.slot = slot,
			.offset = offset,
	...
	GLES3ConstantBuffer3DCommand& GLES3Renderer3DCommandManager::getConstantBuffer(const uint32 index)
	{
		return m_constantBufferCommands[index];
	}

	const __m128* GLES3Renderer3DCommandManager::getConstantBufferPtr(const uint32 offset) const
	{
		return (m_constants.data() + offset);
	}

	void GLES3Renderer3DCommandManager::pushVSTextureUnbind(const uint32 slot)
	{
		assert(slot < SamplerState::MaxSamplerCount);

		const auto id = Texture::IDType::InvalidValue();
		const auto command = ToEnum<GLES3Renderer3DCommandType>(FromEnum(GLES3Renderer3DCommandType::VSTexture0) + slot);
	...
	void GLES3Renderer3DCommandManager::pushVSTexture(const uint32 slot, const Texture& texture)
	{
		assert(slot < SamplerState::MaxSamplerCount);
	...
	const Texture::IDType& GLES3Renderer3DCommandManager::getVSTexture(const uint32 slot, const uint32 index) const
	{
		return m_vsTextures[slot][index];
	}

	const std::array<Texture::IDType, SamplerState::MaxSamplerCount>& GLES3Renderer3DCommandManager::getCurrentVSTextures() const
	{
		return m_currentVSTextures;
	}

	void GLES3Renderer3DCommandManager::pushPSTextureUnbind(const uint32 slot)
	{
		assert(slot < SamplerState::MaxSamplerCount);

		const auto id = Texture::IDType::InvalidValue();
		const auto command = ToEnum<GLES3Renderer3DCommandType>(FromEnum(GLES3Renderer3DCommandType::PSTexture0) + slot);
	...
	void GLES3Renderer3DCommandManager::pushPSTexture(const uint32 slot, const Texture& texture)
	{
		assert(slot < SamplerState::MaxSamplerCount);
	...
	const Texture::IDType& GLES3Renderer3DCommandManager::getPSTexture(const uint32 slot, const uint32 index) const
	{
		return m_psTextures[slot][index];
	}

	const std::array<Texture::IDType, SamplerState::MaxSamplerCount>& GLES3Renderer3DCommandManager::getCurrentPSTextures() const
	{
		return m_currentPSTextures;
	}

	void GLES3Renderer3DCommandManager::pushRT(const Optional<RenderTexture>& rt)
	{
		constexpr auto command = GLES3Renderer3DCommandType::SetRT;
	...
	const Optional<RenderTexture>& GLES3Renderer3DCommandManager::getRT(const uint32 index) const
	{
		return m_RTs[index];
	}
	...
	void GLES3Renderer3DCommandManager::pushMesh(const Mesh& mesh)
	{
		...
		constexpr auto command = GLES3Renderer3DCommandType::SetMesh;
		auto& current = m_currentMesh;
		auto& buffer = m_meshes;
	...
	const Mesh::IDType& GLES3Renderer3DCommandManager::getMesh(const uint32 index) const
	{
