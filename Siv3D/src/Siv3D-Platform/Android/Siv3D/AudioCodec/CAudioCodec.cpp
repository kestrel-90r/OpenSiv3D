...
////                       m_codec_context->ch_layout.u.mask,
///						m_codec_context->sample_fmt, m_codec_context->sample_rate, 0, nullptr);
                ...
                if (!m_swr_context)
                {
                    ...
                    if (!m_swr_context) return false;
                }

                int ret = swr_alloc_set_opts2(&m_swr_context,
                                              &out_ch_layout,                    // 出力チャンネルレイアウト
                                              out_sample_fmt,                    // 出力サンプルフォーマット
                                              m_out_sample_rate,                 // 出力サンプルレート
                                              &m_codec_context->ch_layout,       // 入力チャンネルレイアウト
                                              m_codec_context->sample_fmt,       // 入力サンプルフォーマット
                                              m_codec_context->sample_rate,      // 入力サンプルレート
                                              0,                                 // ログオフセット
                                              nullptr);                          // ログコンテキスト
                if (ret < 0)
                {
